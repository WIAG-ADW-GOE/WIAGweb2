...
- `wiag_persons.csv`: Exported from the WIAG SQL database.
- `pr_persons.csv`: Exported from the PR SQL database.

**Output File**:

- `update_pr.sql`: SQL file to be run on the PR SQL database.

**Instructions**:

1. Obtain the input CSV files:
   - Export `wiag_persons.csv` from the WIAG SQL database.
   - Export `pr_persons.csv` from the PR SQL database.
...
6. Upon completion, the notebook will generate `update_pr.sql`.
7. Run `update_pr.sql` on the PR SQL database to update the WIAG IDs.
